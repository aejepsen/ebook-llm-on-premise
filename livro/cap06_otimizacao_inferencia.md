...
> **Nota sobre GQA (Grouped Query Attention):** Modelos modernos como Llama, Qwen e Mistral usam GQA, onde múltiplos Q-heads compartilham o mesmo par K,V. O número relevante para o KV cache é `num_kv_heads` (heads de K,V), não `num_attention_heads`. Para Llama 3.1 8B: 32 attention heads, mas apenas 8 KV heads.

Para um Llama 3.1 8B (32 camadas, 8 KV heads, dim_head=128, FP16):

```
KV Cache para 4096 tokens = 2 x 32 x 8 x 128 x 4096 x 2 bytes
                          = 536,870,912 bytes
                          ≈ 512 MB
```
...
def medir_requisicao(
    ...
    """
    Envia uma requisicao com streaming e mede as métricas.
    """
    payload = {
        "model": modelo,
        "prompt": prompt,
...
7. Yu, G. et al. (2022). *ORCA: A Continuous Batching Framework for Large Language Model Serving*. OSDI 2022.
8. Wang, C. & Hu, P. (2025). Notebook: `ch09_model_optimization_in_practice.ipynb` — case study de otimização end-to-end do Qwen3-14B (FP16 → GPTQ INT4 → AWQ → GGUF Q4_K_M, 3× comparação, economia de 75% VRAM). Repositório: github.com/orca3/llm-model-serving.
