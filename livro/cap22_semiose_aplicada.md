...
A rodada seguinte (2026-07-02) confirmou a mesma leitura em escala maior: o golden foi expandido para **153 casos** multi-domínio (golden denso), no qual o modelo partia de 72,5%. *Guards* determinísticos e regras de decomposição no prompt levaram a 91,5%, e a auditoria de labels do golden — critérios explícitos, 8 labels inconsistentes normalizados — fechou em **94,1%** (Projeto AI-Orchestrator — `docs/golden_routing_criteria.md`). O padrão se repete: o maior salto (72,5 → 91,5) veio do harness, não do modelo; e o próprio golden, como todo instrumento de medida, precisou de calibração.
